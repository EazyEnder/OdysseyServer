...
public class CommandDungeon implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String arg2, String[] args) {
		...
		if (sender.hasPermission("dungeons.manage")) {
			if (args.length > 0) {
				...
				if (args[0].equals("create")) {
					...
					if (!dungeonFile.exists()) {
						...
						try {
							...
							Dungeon dungeon = new Dungeon(args[1], new HashMap<>(), new HashMap<>(), new HashMap<>(), null, null, new ArrayList<>());
					...
					} else sender.sendMessage("§cCe donjon existe déjà !");
				}
				...
				if (args[0].equals("startloc") && sender instanceof Player) {
					...
					p.sendMessage("§dDonjon " + args[1] + " | pos : " + pos + " -> new location : " + p.getLocation().toString());
				...
				if (args[0].equals("mob")  && sender instanceof Player) {
					...
					sender.sendMessage("§dDonjon " + args[1] + " | mob : §6" + args[2] + " -> new location : §6" + p.getLocation().toString());
					refreshDungeonYaml(d);
