...
public class SQLManager {
	...
	public boolean checkConnection() throws SQLException {
		if (this.connection == null || this.connection.isClosed()) {
			...
			execute("CREATE TABLE IF NOT EXISTS masteries (uuid VARCHAR(36) PRIMARY KEY, warrior float, archer float, tank float, fire float, water float, earth float, wind float, light float, shadow float, classe VARCHAR(20))");
