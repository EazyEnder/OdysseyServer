...
public class SQLManager {
	...
	public boolean checkConnection() throws SQLException {
		if (this.connection == null || this.connection.isClosed()) {
			...
			execute("CREATE TABLE IF NOT EXISTS masteries (uuid VARCHAR(36) PRIMARY KEY, guerrier float, archer float, tank float, mage float, fire float, water float, earth float, wind float, light float, shadow float, classe VARCHAR(20), guerrierXP float, archerXP float, tankXP float, mageXP float)");
